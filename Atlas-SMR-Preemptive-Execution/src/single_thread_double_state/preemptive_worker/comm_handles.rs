    ...
    A: Application<S>,
{
    ...
    pub fn request_latest_confirmed_state(&self, seq_no: SeqNo) -> Result<(SeqNo, S), RequestLatestStateError> {
        if !self.confirmed_worker_rx.is_empty() {
            return Err(RequestLatestStateError::UnexpectedMessage);
        }
        
        self.confirmed_worker_tx.send(PreemptiveToConfirmedMsg::RequestStateCopy(seq_no))?;
        ...
        match self.confirmed_worker_rx.recv() {
            ...
            Err(err) => {
                Err(RequestLatestStateError::ReceiveFailed(err))
            }
        }
    }
}
...
pub(super) enum RequestLatestStateError {
    ...
    #[error("Received unexpected message in confirmed worker while requesting state copy. This channel must be empty when requesting the state copy.")]
    UnexpectedMessage,
}