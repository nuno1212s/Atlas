...
use crate::message::{NetworkMessageKind, PingMessage, System};
          ...
          W: Write
{
    ...
    let mut network_message: febft_capnp::network_messages_capnp::network_message::Builder = root.init_root();

    match m {
        NetworkMessageKind::Ping(ping_message) => {
            let mut ping_message_builder = network_message.reborrow().init_ping_message();

            ping_message_builder.set_request(ping_message.is_request());
        }
